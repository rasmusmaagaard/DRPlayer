	...
	for (unsigned int i = 0; i < elementCount; i++) {
		switch ([self elementAtIndex:i associatedPoints:controlPoints]) {
			...
			case NSClosePathBezierPathElement:
				...
				break;
			default:
				DebugNSLog(@"Unknown element at [NSBezierPath (GTMBezierPathCGPathAdditions) cgPath]");
				break;
		};
	}
	return thePath;
...
- (NSBezierPath *)pathWithStrokeWidth:(CGFloat)strokeWidth
{
    ...
    NSAssert(context, @"NSBezierPath+MCAdditions");
...
- (void)fillWithInnerShadow:(NSShadow *)shadow
{
    ...
    NSAssert([NSGraphicsContext currentContext], @"NSBezierPath+MCAdditions");
...
- (void)strokeInsideWithinRect:(NSRect)clipRect
{
    ...
    NSAssert(thisContext, @"NSBezierPath+MCAdditions");
