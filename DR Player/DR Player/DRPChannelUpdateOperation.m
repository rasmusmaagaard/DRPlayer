...
-(void)main
{
    @autoreleasepool
    {
    	if (![self isCancelled])
        {
            NSMutableDictionary *streamDict = [NSMutableDictionary dictionaryWithContentsOfURL:[[NSURL URLWithString:[NSString remoteResourcesURLString]] URLByAppendingPathComponent:@"DRPChannels.plist"]];
            ...
            if (streamDict)
            {
                ...
                for (NSDictionary *channelInfo in sourceArray)
                {
                    ...
                    newChannel.tintColor        = [NSColor colorFromHexRGB: channelInfo[DRPChannelTintColorKey] ? channelInfo[DRPChannelTintColorKey]:[NSColor clearColor]];
